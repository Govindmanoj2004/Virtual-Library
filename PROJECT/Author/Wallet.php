...
{
    $withdrawAmt=$_POST['hidden_withdraw'];
    $auid=$_SESSION['auid'];
    $insQuery="insert into tbl_wallet (wallet_date,wallet_withdrawel,author_id) values (curdate(),$withdrawAmt,$auid)";
    if($withdrawAmt<=0)
    {
        ?>
        <script>
...
<table width="200" border="1" align="center">
  <tr>
   ...
   <?php
  	$stocksel="select * from tbl_wallet where author_id=".$_SESSION['auid'];
	$result = $con->query($stocksel);
	$i=0;
    $credited=0;
    $withdraw=0;
  ...
  <tr>
    <td><?php echo $i ?></td>
    <td><?php echo $data['wallet_date'] ?></td>
    <td><?php echo $data['wallet_credited'] ?></td>
    <?php
    $credited=$credited+$data['wallet_credited'];
    $withdraw=$withdraw+$data['wallet_withdrawel'];
   ...
   <?php
    ...
    <tr>
        <td colspan="2" align="center">
            <p>Balance:<?php echo $balance=$credited-$withdraw ?></p>
            <p>Amount withdrawed:<?php echo $withdraw ?></td>
        <td>
            <?php
            if($balance==0)
            {
            ...
            {
                ?>
                <input type="button" name="sub_withdraw" id="sub_withdraw" value="Withdraw" /></td>
                <?php
            }
            ?>
...
<link rel="stylesheet" href="../Assets/Templates/Wallet/style.css" /><link rel="stylesheet" href="../Assets/Templates/Wallet/style.css">
  ...
  <form class="form" method="POST">
    <div>
      ...
      <div class="card">
        <address>
            <?php
            $selQuery="select * from tbl_author au inner join tbl_district d on d.district_id=au.author_district inner join tbl_place p on p.place_id=au.author_place where author_id=".$_SESSION['auid'];
            $selResult=$con->query($selQuery);
            $selData=$selResult->fetch_assoc();
            ?>
          <?php echo $selData['author_name'] ?><br />
          <?php echo $selData['author_address'] ?><br />
          <?php echo $selData['place_name'] ?><br />
          <?php echo $selData['district_name'] ?>
        </address>
      </div>
    </div>
      ...
      <table>
        <tbody>
          <tr>
            ...
            <td align="right">&#8377 <?php echo $credited ?></td>
          </tr>
          <tr>
            <td>Amount withdrawed</td>
            <td align="right">-&#8377 <?php echo $withdraw ?></td>
          </tr>
        <input type="hidden" name="hidden_withdraw" id="hidden_withdraw" value="<?php echo $balance ?>" />        
        </tbody>
        <tfoot>
          <tr>
            <td>Balance</td>
            <td align="right">&#8377 <?php echo $balance ?></td>
          </tr>
        </tfoot>
      </table>
    </div>
    ...
    <div>
        <!-- <input type="submit" class="button button--full" name="sub_withdraw" id="sub_withdraw" value="Withdraw" /> -->
        <button class="button button--full" type="submit" name="Button" id="Button" >Withdraw </button>
    </div>
  </form>
</div>
...
<!-- partial -->
  <script  src="../Assets/Templates/Wallet/script.js"></script>
