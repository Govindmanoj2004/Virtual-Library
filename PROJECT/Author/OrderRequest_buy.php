...
if(isset($_GET['updt']))
{
  $uptStatus="update tbl_booking set booking_status=booking_status+1 where booking_id=".$_GET['updt'];
  if($con->query($uptStatus))
  {
    ?>
    <script>
    alert('Booking status updated')
    window.location='OrderRequest_buy.php'
    </script>
    <?php
  }
  else
  {
    ?>
    <script>
    alert('Error:Booking status not updated')
    window.location='OrderRequest_buy.php'
    </script>
    <?php
  }
...
<title>Order Request(BUy)</title>
</head>

<body>
<h2 align="center">Order Request(Booking)</h2>
<table width="532" height="82" border="1" align="center">
  <tr>
    <td>Sl.no</td>
    <td>Book Name</td>
    <td>Book Cover</td>
  ...
  <?php
    $slctRqst="select * from tbl_booking b inner join tbl_books a on a.book_id=b.book_id where a.book_author='".$authorId."'";
  ...
  <tr>
    ...
    <td>
      ...
      {
        ?>
        <a href="OrderRequest_buy.php?updt=<?php echo $data["booking_id"] ?>">Change status to Confirm Order</a></td>
        <?php
      }
      else if($data['booking_status']==2)
      {
        ?>
        <a href="OrderRequest_buy.php?updt=<?php echo $data["booking_id"] ?>">Change status to order shipped</a></td>
        <?php
      }
      else if($data['booking_status']==3)
      {
        ?>
        <a href="OrderRequest_buy.php?updt=<?php echo $data["booking_id"] ?>">Change status to out for Delivery</a></td>
        <?php
      }
      else if($data['booking_status']==4)
      {
        ?>
        <a href="OrderRequest_buy.php?updt=<?php echo $data["booking_id"] ?>">Change status to order delivered</a></td>
        <?php
      }
      ...
      ?>
      <td><a href="ViewComplaints.php?comp=<?php echo $data["book_id"] ?>">View Complaints</a></td>
  </tr>
  <?php
    }
  ?>
			...
			<div class="row s-header__navigation">
				...
				<nav class="s-header__nav-wrap">
					...
					<ul class="s-header__nav">
                        ...
                        <?php
                        $slct="select * from tbl_subscription where author_id='".$_SESSION['auid']."'";
			...
			<div class="s-pageheader">
				<div class="row">
					<div class="column large-12">
						<h1 class="page-title">
							<span class="page-title__small-type">Listings.</span>
							Ordered
						</h1>
					</div>
				</div>
...
<div class="masonry">
	...
	<div class="bricks-wrapper" data-animate-block>
		...
		<?php
		$slBooks="select * from tbl_booking r inner join tbl_books a on a.book_id=r.book_id inner join tbl_genre g on g.genre_id=a.book_genre inner join tbl_category ca on ca.cat_id=a.book_cat where a.book_author='".$_SESSION['auid']."'";
		...
		<article class="brick entry" data-animate-el>
			...
			<div class="entry__thumb">
				<a href="Viewmore.php?bookid=<?php echo $bookData["book_id"] ?>" class="thumb-link">
					<img src="../Assets/Files/Author/BookCover/<?php echo $bookData['book_cover'] ?>" 
						srcset="../Assets/Files/Author/BookCover/<?php echo $bookData['book_cover'] ?> 1x, Author/BookCover/<?php echo $bookData['book_cover'] ?> 2x" alt="">
				</a>
				...
				<div class="entry__header">
					<div class="entry__meta">
						<span class="cat-links">
							<a href="#"><?php echo $bookData['cat_name'].",".$bookData['genre_name'] ?></a>
						</span>
						<span class="byline">
            <?php
            ...
            else if($bookData['booking_status']==2)
            {
              ?>
              Status:Confirm Order
              <?php
            }
            else if($bookData['booking_status']==3)
            {
              ?>
            ...
            else if($bookData['booking_status']==4)
            {
              ?>
              Status:Out for Delivery
              <?php
            }
            else if($bookData['booking_status']==5)
            {
              ?>
            ...
            ?>
				...
				<div class="entry__excerpt">
					<p>
						<?php 
            $max_length = 300; // Set the maximum length of the paragraph
            $book_des = $bookData ['book_des'];
        ...
        {
          ?>
          <a class="entry__more-link" href="OrderRequest_buy.php?updt=<?php echo $bookData['booking_id'] ?>">Update Order</a>
          <?php
        }
        else if($bookData['booking_status']==2)
        {
        ...
        <div>
        <a class="entry__more-link" href="ViewComplaints.php?comp=<?php echo $bookData['book_id'] ?>">View Complaint</a>
        </div>
			</div> <!-- end entry__text -->
