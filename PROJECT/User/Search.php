<?php
include("../Assets/Connection/connection.php");

if(isset($_GET['lst']))
{
	$bId=$_GET['lst'];
	$bookSelct="select * from tbl_wishlist where book_id='$bId' and user_id='$id'";
	$result=$con->query($bookSelct);
	if($result->num_rows>0)
	{
		$wishRemove="delete from tbl_wishlist where book_id='$bId' and user_id='$id'";
		$remove=$con->query($wishRemove);
		?>
<script>
	alert('Removed from wishlist')
	window.location = 'Booklist.php'
</script>
<?php
		
	}
	else
	{
		$bookIns="insert into tbl_wishlist (book_id,user_id) values ('$bId','$id')";
		if($con->query($bookIns))
		{
			?>
<script>
	alert('Book Added To Wishlist')
	window.location = 'Booklist.php'
</script>
<?php
		}
		else
		{
			?>
<script>
	alert('Book Not Added')
	window.location = 'Booklist.php'
</script>
<?php
		}
	}
}
?>
<!DOCTYPE html>
<html lang="en" class="no-js">

<head>

	<!--- basic page needs
    ================================================== -->
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<title>Search</title>

	<script>
		document.documentElement.classList.remove('no-js');
		document.documentElement.classList.add('js');
	</script>

	<!-- CSS
    ================================================== -->
	<link rel="stylesheet" href="../Assets/Templates/Main/css/vendor.css">
	<link rel="stylesheet" href="../Assets/Templates/Main/css/styles.css">

	<!-- Modal -->
	<style>
		.filter-button {
			padding: 10px 20px;
			font-size: 16px;
			cursor: pointer;
		}

		.modal {
			display: none;
			position: fixed;
			z-index: 1;
			left: 0;
			top: 0;
			width: 100%;
			height: 100%;
			overflow: auto;
			background-color: rgb(0, 0, 0);
			background-color: rgba(0, 0, 0, 0.4);
		}

		.modal-content {
			background-color: #fefefe;
			margin: 5% auto;
			padding: 20px;
			border: 1px solid #888;
			width: 1200px;
			height: 650px;
			overflow-x: scroll;
			border-radius: 30px;
		}

		.close {
			color: #aaa;
			float: right;
			font-size: 28px;
			font-weight: bold;
		}

		.close:hover,
		.close:focus {
			color: black;
			text-decoration: none;
			cursor: pointer;
		}

		.checkboxes {
			display: flex;
			gap: 1rem;
			padding-left: 1rem;
		}

		.sform {
			display: flex !important;
			justify-content: center !important;
			gap: 15px;
		}

		.checkboxes span {
			font-size: 20px;
		}

		.modalHead {
			font-size: 30px !important;
			margin-bottom: 30px;
			font-family: "inter", san-serif;
			/* font-weight: 700px; */
		}

		.modalFilter {
			display: flex;
			flex-direction: column;
			margin-top: 30px;
			margin-left: 5px;
		}

		.labelModal {
			font-weight: 400 !important;
		}

		/* Content Edit */

		.entry__thumb img {
			width: 100%;
			/* Ensure the image fills its container */
			height: 450px;
			/* Fixed height for images */
			object-fit: cover;
			/* Ensure the image fits nicely within the given dimensions */
		}

		.entry__title {
			font-size: 1.2em;
			/* Consistent font size for the title */
			height: 3em;
			/* Fixed height for title to limit to two lines */
			overflow: hidden;
			/* Hide overflow text */
			text-overflow: ellipsis;
			/* Add ellipsis to overflow text */
			white-space: nowrap;
			/* Prevent text from wrapping */
		}

		.entry__except p {
			font-size: 0.9em;
			/* Consistent font size for the description */
			max-height: 4.5em;
			/* Set a maximum height to limit description length */
			overflow: hidden;
			/* Hide overflow text */
			text-overflow: ellipsis;
			/* Add ellipsis to overflow text */
			line-height: 1.5em;
			/* Set line height for the text */
		}

		.brick.entry {
			/* max-width: 300px; Set a maximum width for the entire entry */
			margin: 0 auto;
			/* Center the entries */
		}

		/* check boxes  in modals */
		.checkboxes {
			display: inline-block;
			margin: 5px;
		}

		.checkboxes input[type="checkbox"] {
			display: none;
			/* Hide the actual checkbox */
		}

		.checkboxes label {
			display: flex;
			padding: 10px 20px;
			border: 2px solid black;
			border-radius: 4px;
			background-color: white;
			color: black;
			cursor: pointer;
			font-family: var(--font-1);
			transition: background-color 0.3s ease, color 0.3s ease;
			margin-bottom: 0 !important;
		}

		.checkboxes input[type="checkbox"]:checked+label {
			background-color: #007bff;
			color: #fff;
			border-color: #007bff;
		}

		.modalButtons {
			display: flex;
			gap: 1rem;
			flex-wrap: wrap;
			justify-content: space-evenly;

		}

		.catList {
			display: flex;
			flex-direction: column;
			gap: 1rem;
			flex-wrap: nowrap;
			width: 270px;
			/* Set the width of the rectangle */
			height: 400px;
			/* Set the height of the rectangle */
			/* overflow-y: auto; Allows vertical scrolling if content exceeds height */
			padding: 1rem;
			box-sizing: border-box;
		}

		.catHead {
			font-size: 30px;
			display: inLine-block;
		}

		.genreList {
			display: flex;
			flex-wrap: wrap;
			gap: 18px;
			justify-content: space-evenly;
		}

		.noGenre {
			font-size: 23px;
		}
	</style>

	<!-- favicons
    ================================================== -->
	<link rel="apple-touch-icon" sizes="180x180" href="../Assets/Templates/Main/apple-touch-icon.png">
	<link rel="icon" type="image/png" sizes="32x32" href="../Assets/Templates/Main/favicon-32x32.png">
	<link rel="icon" type="image/png" sizes="16x16" href="../Assets/Templates/Main/favicon-16x16.png">
	<!-- <link rel="manifest" href="../Assets/Templates/Main/site.webmanifest"> -->

</head>

<body id="top" onload="Search()">


	<!-- preloader
    ================================================== -->
	<!-- <div id="preloader">
		<div id="loader" class="dots-fade">
			<div></div>
			<div></div>
			<div></div>
		</div>
	</div> -->


	<!-- page wrap
    ================================================== -->
	<div id="page" class="s-pagewrap">


		<!-- # site header 
        ================================================== -->
		<header id="masthead" class="s-header">

			<div class="s-header__branding">
				<p class="site-title">
					<a href="Homepage.php" rel="home">Virtual library.</a>
				</p>
			</div>

			<div class="row s-header__navigation">

				<nav class="s-header__nav-wrap">

					<h3 class="s-header__nav-heading">Navigate to</h3>

					<ul class="s-header__nav">
						<li class="current-menu-item"><a href="Homepage.php" title="">Home</a></li>
						<li><a href="Login.php" title="">Login</a></li>
						<li class="has-children">
							<a href="#0" title="" class="">Sign in</a>
							<ul class="sub-menu">
								<li><a href="User_reg.php">Sign in as User</a></li>
								<li><a href="Author_reg.php">Sign in as Author/Seller</a></li>
							</ul>
						</li>
						<li><a href="Booklist.php" title="">Library</a></li>
						<li class="has-children">
							<a href="#0" title="" class="">Support</a>
							<ul class="sub-menu">
								<li><a href="../Assets/Templates/Main/contact.html" title="">Contact</a></li>
								<li><a href="../Assets/Templates/Main/about.html" title="">About</a></li>
							</ul>
						</li>
					</ul> <!-- end s-header__nav -->

				</nav> <!-- end s-header__nav-wrap -->

			</div> <!-- end s-header__navigation -->

			<div class="s-header__search">

				<div class="s-header__search-inner">
					<div class="row">

						<form role="search" method="get" class="s-header__search-form" action="#">
							<label>
								<span class="u-screen-reader-text">Search for:</span>
								<input type="search" class="s-header__search-field" placeholder="Search for..." value=""
									name="s" title="Search for:" autocomplete="off">
							</label>
							<input type="submit" class="s-header__search-submit" value="Search">
						</form>

						<a href="#0" title="Close Search" class="s-header__search-close">Close</a>

					</div> <!-- end row -->
				</div> <!-- s-header__search-inner -->

			</div> <!-- end s-header__search -->

			<a class="s-header__menu-toggle" href="#0"><span>Menu</span></a>
			<a class="s-header__search-trigger" href="Search.php">
				<svg width="24" height="24" fill="none" viewBox="0 0 24 24">
					<path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
						d="M19.25 19.25L15.5 15.5M4.75 11C4.75 7.54822 7.54822 4.75 11 4.75C14.4518 4.75 17.25 7.54822 17.25 11C17.25 14.4518 14.4518 17.25 11 17.25C7.54822 17.25 4.75 14.4518 4.75 11Z">
					</path>
				</svg>
			</a>

		</header> <!-- end s-header -->


		<!-- # site-content
        ================================================== -->
		<section id="content" class="s-content">


			<!-- pageheader -->
			<div class="s-pageheader">
				<div class="row">
					<div class="column large-12">
						<h1 class="page-title">
							<form method="post" class="sform">
								<input name="txt_name" type="text" id="txt_search" placeholder="Search"
									onkeyup="Search()" class="u-fullwidth text-center" />
								<button onclick="document.getElementById('myModal').style.display='block'" type="button"
									class="btn btn--stroke">Filter</button>



								<div id="myModal" class="modal">
									<div class="modal-content">
										<span class="close"
											onclick="document.getElementById('myModal').style.display='none'">&times;
										</span>
										<!-- <h4 align="center">Genre</h4> -->
										<!-- Your filter options go here -->
										<?php
                              			$slctCat = "select * from tbl_category";
                              			$resultCat=$con->query($slctCat);
                              			?>
										<div class="modalFilter">
											<span class="modalHead">CATEGORY</span>
											<div class="modalButtons">
												<?php
                              				while($dataCat = $resultCat->fetch_assoc())
                              				{
                              				  ?>
												<div class="checkboxes">
													<input type="checkbox" id="cat<?php echo $dataCat['cat_id'] ?>"
														name="cat[]" onclick="addCat(<?php echo $dataCat['cat_id'] ?>)"
														value="<?php echo $dataCat['cat_id'] ?>">
													<label for="cat<?php echo $dataCat['cat_id'] ?>" class="labelModal">
														<span>
															<?php echo $dataCat['cat_name'] ?>
														</span>
													</label>
												</div>

												<?php
                              				}
                              				?>
											</div>
										</div>
										<!-- Your filter options go here -->
										<?php
                              			$dissel = "select * from tbl_genre";
                              			$result=$con->query($dissel);
                              			$i=0;
                              			?>
										<div class="modalFilter">
											<span class="modalHead">GENRE</span>
											<!-- <br /> -->
											<div class="genreList" id="genreList">

											</div>

										</div>
									</div>
								</div>
							</form>
						</h1>
					</div>
				</div>
			</div> <!-- end s-pageheader-->

			<!--  masonry -->
			<div id="bricks" class="bricks">

				<div class="masonry" id="search">


				</div> <!-- end bricks -->

			</div>

		</section> <!-- end s-content -->


		<!-- # site-footer
        ================================================== -->
		<footer id="colophon" class="s-footer">

			<div class="row s-footer__subscribe">
				<div class="column lg-12">

					<h2>
						Subscribe to Our Newsletter.
					</h2>
					<p>
						Subscribe now to get all latest updates
					</p>

					<form id="mc-form" class="mc-form">
						<input type="email" name="EMAIL" id="mce-EMAIL" class="u-fullwidth text-center"
							placeholder="Your Email Address"
							title="The domain portion of the email address is invalid (the portion after the @)."
							pattern="^([^\x00-\x20\x22\x28\x29\x2c\x2e\x3a-\x3c\x3e\x40\x5b-\x5d\x7f-\xff]+|\x22([^\x0d\x22\x5c\x80-\xff]|\x5c[\x00-\x7f])*\x22)(\x2e([^\x00-\x20\x22\x28\x29\x2c\x2e\x3a-\x3c\x3e\x40\x5b-\x5d\x7f-\xff]+|\x22([^\x0d\x22\x5c\x80-\xff]|\x5c[\x00-\x7f])*\x22))*\x40([^\x00-\x20\x22\x28\x29\x2c\x2e\x3a-\x3c\x3e\x40\x5b-\x5d\x7f-\xff]+|\x5b([^\x0d\x5b-\x5d\x80-\xff]|\x5c[\x00-\x7f])*\x5d)(\x2e([^\x00-\x20\x22\x28\x29\x2c\x2e\x3a-\x3c\x3e\x40\x5b-\x5d\x7f-\xff]+|\x5b([^\x0d\x5b-\x5d\x80-\xff]|\x5c[\x00-\x7f])*\x5d))*(\.\w{2,})+$"
							required>
						<input type="submit" name="subscribe" value="Subscribe"
							class="btn--small btn--primary u-fullwidth">
						<!-- <div style="position: absolute; left: -5000px;" aria-hidden="true"><input type="text" name="b_cdb7b577e41181934ed6a6a44_9a91cfe7b3" tabindex="-1" value=""></div> -->
						<div class="mc-status"></div>
					</form>

				</div>
			</div> <!-- end s-footer__subscribe -->

			<div class="row s-footer__main">

				<div class="column lg-5 md-6 tab-12 s-footer__about">
					<h4>Spurgeon</h4>

					<p>
						Lorem ipsum dolor sit amet, consectetur
						adipiscing elit, sed do eiusmod tempor
						incididunt ut labore et dolore magna aliqua.
						Ut enim ad minim veniam, quis nostrud exercitation
						ullamco laboris nisi ut aliquip ex ea commodo
					</p>
				</div> <!-- end s-footer__about -->

				<div class="column lg-5 md-6 tab-12">
					<div class="row">
						<div class="column lg-6">
							<h4>Categories</h4>
							<ul class="link-list">
								<li><a href="category.html">Lifestyle</a></li>
								<li><a href="category.html">Workplace</a></li>
								<li><a href="category.html">Inspiration</a></li>
								<li><a href="category.html">Design</a></li>
								<li><a href="category.html">Health</a></li>
								<li><a href="category.html">Photography</a></li>
							</ul>
						</div>
						<div class="column lg-6">
							<h4>Site Links</h4>
							<ul class="link-list">
								<li><a href="index.html">Home</a></li>
								<li><a href="category.html">Categories</a></li>
								<li><a href="category.html">Blog</a></li>
								<li><a href="about.html">About</a></li>
								<li><a href="about.html">Contact</a></li>
								<li><a href="#0">Terms & Policy</a></li>
							</ul>
						</div>
					</div>
				</div>

			</div> <!-- end s-footer__main -->

			<div class="row s-footer__bottom">

				<div class="column lg-7 md-6 tab-12">
					<ul class="s-footer__social">
						<li>
							<a href="#0">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
									style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;">
									<path
										d="M12.001 2.002c-5.522 0-9.999 4.477-9.999 9.999 0 4.99 3.656 9.126 8.437 9.879v-6.988h-2.54v-2.891h2.54V9.798c0-2.508 1.493-3.891 3.776-3.891 1.094 0 2.24.195 2.24.195v2.459h-1.264c-1.24 0-1.628.772-1.628 1.563v1.875h2.771l-.443 2.891h-2.328v6.988C18.344 21.129 22 16.992 22 12.001c0-5.522-4.477-9.999-9.999-9.999z">
									</path>
								</svg>
								<span class="u-screen-reader-text">Facebook</span>
							</a>
						</li>
						<li>
							<a href="#0">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
									style="fill:rgba(0, 0, 0, 1);transform:;-ms-filter:">
									<path
										d="M19.633,7.997c0.013,0.175,0.013,0.349,0.013,0.523c0,5.325-4.053,11.461-11.46,11.461c-2.282,0-4.402-0.661-6.186-1.809 c0.324,0.037,0.636,0.05,0.973,0.05c1.883,0,3.616-0.636,5.001-1.721c-1.771-0.037-3.255-1.197-3.767-2.793 c0.249,0.037,0.499,0.062,0.761,0.062c0.361,0,0.724-0.05,1.061-0.137c-1.847-0.374-3.23-1.995-3.23-3.953v-0.05 c0.537,0.299,1.16,0.486,1.82,0.511C3.534,9.419,2.823,8.184,2.823,6.787c0-0.748,0.199-1.434,0.548-2.032 c1.983,2.443,4.964,4.04,8.306,4.215c-0.062-0.3-0.1-0.611-0.1-0.923c0-2.22,1.796-4.028,4.028-4.028 c1.16,0,2.207,0.486,2.943,1.272c0.91-0.175,1.782-0.512,2.556-0.973c-0.299,0.935-0.936,1.721-1.771,2.22 c0.811-0.088,1.597-0.312,2.319-0.624C21.104,6.712,20.419,7.423,19.633,7.997z">
									</path>
								</svg>
								<span class="u-screen-reader-text">Twitter</span>
							</a>
						</li>
						<li>
							<a href="#0">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
									style="fill:rgba(0, 0, 0, 1);transform:;-ms-filter:">
									<path
										d="M11.999,7.377c-2.554,0-4.623,2.07-4.623,4.623c0,2.554,2.069,4.624,4.623,4.624c2.552,0,4.623-2.07,4.623-4.624 C16.622,9.447,14.551,7.377,11.999,7.377L11.999,7.377z M11.999,15.004c-1.659,0-3.004-1.345-3.004-3.003 c0-1.659,1.345-3.003,3.004-3.003s3.002,1.344,3.002,3.003C15.001,13.659,13.658,15.004,11.999,15.004L11.999,15.004z">
									</path>
									<circle cx="16.806" cy="7.207" r="1.078"></circle>
									<path
										d="M20.533,6.111c-0.469-1.209-1.424-2.165-2.633-2.632c-0.699-0.263-1.438-0.404-2.186-0.42 c-0.963-0.042-1.268-0.054-3.71-0.054s-2.755,0-3.71,0.054C7.548,3.074,6.809,3.215,6.11,3.479C4.9,3.946,3.945,4.902,3.477,6.111 c-0.263,0.7-0.404,1.438-0.419,2.186c-0.043,0.962-0.056,1.267-0.056,3.71c0,2.442,0,2.753,0.056,3.71 c0.015,0.748,0.156,1.486,0.419,2.187c0.469,1.208,1.424,2.164,2.634,2.632c0.696,0.272,1.435,0.426,2.185,0.45 c0.963,0.042,1.268,0.055,3.71,0.055s2.755,0,3.71-0.055c0.747-0.015,1.486-0.157,2.186-0.419c1.209-0.469,2.164-1.424,2.633-2.633 c0.263-0.7,0.404-1.438,0.419-2.186c0.043-0.962,0.056-1.267,0.056-3.71s0-2.753-0.056-3.71C20.941,7.57,20.801,6.819,20.533,6.111z M19.315,15.643c-0.007,0.576-0.111,1.147-0.311,1.688c-0.305,0.787-0.926,1.409-1.712,1.711c-0.535,0.199-1.099,0.303-1.67,0.311 c-0.95,0.044-1.218,0.055-3.654,0.055c-2.438,0-2.687,0-3.655-0.055c-0.569-0.007-1.135-0.112-1.669-0.311 c-0.789-0.301-1.414-0.923-1.719-1.711c-0.196-0.534-0.302-1.099-0.311-1.669c-0.043-0.95-0.053-1.218-0.053-3.654 c0-2.437,0-2.686,0.053-3.655c0.007-0.576,0.111-1.146,0.311-1.687c0.305-0.789,0.93-1.41,1.719-1.712 c0.534-0.198,1.1-0.303,1.669-0.311c0.951-0.043,1.218-0.055,3.655-0.055c2.437,0,2.687,0,3.654,0.055 c0.571,0.007,1.135,0.112,1.67,0.311c0.786,0.303,1.407,0.925,1.712,1.712c0.196,0.534,0.302,1.099,0.311,1.669 c0.043,0.951,0.054,1.218,0.054,3.655c0,2.436,0,2.698-0.043,3.654H19.315z">
									</path>
								</svg>
								<span class="u-screen-reader-text">Instagram</span>
							</a>
						</li>
						<li>
							<a href="#0">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
									style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;">
									<path
										d="M11.99 2C6.472 2 2 6.473 2 11.99c0 4.232 2.633 7.85 6.35 9.306-.088-.79-.166-2.006.034-2.868.182-.78 1.172-4.966 1.172-4.966s-.299-.599-.299-1.484c0-1.388.805-2.425 1.808-2.425.853 0 1.264.64 1.264 1.407 0 .858-.546 2.139-.827 3.327-.235.994.499 1.805 1.479 1.805 1.775 0 3.141-1.872 3.141-4.575 0-2.392-1.719-4.064-4.173-4.064-2.843 0-4.512 2.132-4.512 4.335 0 .858.331 1.779.744 2.28a.3.3 0 0 1 .069.286c-.076.315-.245.994-.277 1.133-.044.183-.145.222-.335.134-1.247-.581-2.027-2.405-2.027-3.871 0-3.151 2.289-6.045 6.601-6.045 3.466 0 6.159 2.469 6.159 5.77 0 3.444-2.171 6.213-5.184 6.213-1.013 0-1.964-.525-2.29-1.146l-.623 2.374c-.225.868-.834 1.956-1.241 2.62a10 10 0 0 0 2.958.445c5.517 0 9.99-4.473 9.99-9.99S17.507 2 11.99 2">
									</path>
								</svg>
								<span class="u-screen-reader-text">Pinterest</span>
							</a>
						</li>
					</ul>
				</div>
				<div class="column lg-5 md-6 tab-12">
					<div class="ss-copyright">
						<span>© Copyright Spurgeon 2021</span>
						<span>Design by <a href="https://www.styleshout.com/">StyleShout</a> Distribution <a
								href="https://themewagon.com">ThemeWagon</a></span>
					</div>
				</div>

			</div> <!-- end s-footer__bottom -->

			<div class="ss-go-top">
				<a class="smoothscroll" title="Back to Top" href="#top">
					<svg width="24" height="24" fill="none" viewBox="0 0 24 24">
						<path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
							d="M17.25 10.25L12 4.75L6.75 10.25" />
						<path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
							d="M12 19.25V5.75" />
					</svg>
				</a>
			</div> <!-- end ss-go-top -->

		</footer><!-- end s-footer -->


		<!-- Java Script
    ================================================== -->
		<!-- <script src="../Assets/Templates/Main/js/plugins.js"></script> -->
		<!-- <script src="../Assets/Templates/Main/js/main.js"></script> -->
		<script src="../Assets/JQ/jQuery.js"></script>
		<script>

			function get_filter_text() {
				var filterData = [];

				$('#genre:checked').each(function () {
					filterData.push($(this).val());
				});

				return filterData;
			}

			function get_filter_cat() {
				var filterCat = [];

				$('#cat:checked').each(function () {
					filterCat.push($(this).val());
				});

				return filterCat;
			}

			var catList = [];
			var genreList = [];

			function addCat(id) {
				// Find the index of the ID in the array
				const index = catList.indexOf(id);

				if (index === -1) {
					// ID is not in the array, so add it
					catList.push(id);
				} else {
					// ID is already in the array, so remove it
					catList.splice(index, 1);
				}
				ajaxGenre();
				// Call the Search function
				Search();

			}

			function addGenre(id) {
				// Find the index of the ID in the array
				const index = genreList.indexOf(id);

				if (index === -1) {
					// ID is not in the array, so add it
					genreList.push(id);
				} else {
					// ID is already in the array, so remove it
					genreList.splice(index, 1);
				}
				// ajaxG	enre();
				// Call the Search function
				Search();

			}

			function ajaxGenre() {
				$.ajax({
					url: "../Assets/AjaxPages/AjaxSGenre.php?cat=" + catList,
					success: function (result) {
						$("#genreList").html(result);
					}
				});
			}

			function Search() {
				var book = document.getElementById('txt_search').value;
				// var genre = document.getElementById('lst_genre').value;
				var category = catList;
				var genre = genreList;

				console.log("Genre: " + genre);
				console.log("Category: " + category);

				$.ajax({
					url: "../Assets/AjaxPages/AjaxSearch.php?book=" + book + "&genre=" + genre + "&category=" + category,
					success: function (result) {
						$("#search").html(result);
					}
				});
			}

			// Modal
			// Get the modal
			var modal = document.getElementById("myModal");

			// Get the button that opens the modal
			var btn = document.querySelector(".filter-button");

			// Get the <span> element that closes the modal
			var span = document.getElementsByClassName("close")[0];

			// When the user clicks the button, open the modal 
			// btn.onclick = function () {
			// 	modal.style.display = "block";
			// }

			// When the user clicks on <span> (x), close the modal
			span.onclick = function () {
				modal.style.display = "none";
			}

			// When the user clicks anywhere outside of the modal, close it
			window.onclick = function (event) {
				if (event.target == modal) {
					modal.style.display = "none";
				}
			}


		</script>

</body>

</html>